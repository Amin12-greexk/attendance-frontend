...
export default function ManualEntryPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
        ...
        // Perbaikan: Baca cookie dengan nama yang benar ('XSRF-TOKEN')
        ...
        const fullClockOut = clockOutDate && clockOutTime ? `${format(clockOutDate, 'yyyy-MM-dd')} ${clockOutTime}:00` : null;
        ...
        if (axios.isAxiosError(error) && error.response?.status === 422) {
            ...
            console.error(error);
            const errorMessage = error instanceof Error ? error.message : "Gagal menyimpan data absensi.";
        ...
        <CardContent className="space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-2">
                ...
                <Popover>
                    <PopoverTrigger asChild>
                    <Button variant={"outline"} className={cn("w-full justify-start text-left font-normal", !clockInDate && "text-muted-foreground")}>
                        <CalendarIcon className="mr-2 h-4 w-4" />
                        {clockInDate ? format(clockInDate, "PPP") : <span>Pilih tanggal</span>}
                    </Button>
                    </PopoverTrigger>
                    <PopoverContent className="w-auto p-0"><Calendar mode="single" selected={clockInDate} onSelect={setClockInDate} initialFocus /></PopoverContent>
                </Popover>
            </div>
            <div className="space-y-2">
                <Label htmlFor="clockInTime">Waktu Clock In (HH:MM)</Label>
                <Input id="clockInTime" type="time" value={clockInTime} onChange={(e) => setClockInTime(e.target.value)} />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-2">
                ...
                <Popover>
                    ...
                    <PopoverContent className="w-auto p-0"><Calendar mode="single" selected={clockOutDate} onSelect={setClockOutDate} /></PopoverContent>
                </Popover>
            </div>
            <div className="space-y-2">
                <Label htmlFor="clockOutTime">Waktu Clock Out (HH:MM)</Label>
                <Input id="clockOutTime" type="time" value={clockOutTime} onChange={(e) => setClockOutTime(e.target.value)} />
            </div>
          </div>
